...
export class Guard {
  ...
  public static againstAtMost(numChars: number, text: string): Result<GuardResponse> {
    return text.length <= numChars
      ? Result.ok<GuardResponse>()
      : Result.fail<GuardResponse>(`Text is greater than ${numChars} characters.`);
  }

  public static againstOnlyWhitespace(text: string, argumentName: string): Result<GuardResponse> {
    const isOnlyWhitespace = /^\s*$/.test(text);

    if (isOnlyWhitespace) {
      return Result.fail<GuardResponse>(`${argumentName} contains only whitespace characters.`);
    } else {
      return Result.ok<GuardResponse>();
    }
  }
